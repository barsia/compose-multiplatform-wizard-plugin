...
object ProjectCreator {
    ...
    internal fun createProjectStructure(
        ...
        return try {
            ...
            val config = io.github.barsia.composewizard.generator.ProjectConfig(
            ...
            val composer = io.github.barsia.composewizard.composer.ProjectComposer(config)
            
            if (builder.targetAndroid) {
                composer.addModule(io.github.barsia.composewizard.composer.modules.AndroidModule())
            }
            if (builder.targetIOS) {
                composer.addModule(io.github.barsia.composewizard.composer.modules.iOSModule())
            }
            if (builder.targetDesktop) {
                composer.addModule(io.github.barsia.composewizard.composer.modules.DesktopModule())
            }
            if (builder.targetWeb) {
                composer.addModule(io.github.barsia.composewizard.composer.modules.WebModule())
            }
            ...
            val shouldIncludeHotReloadFeature = when {
                ...
                io.github.barsia.composewizard.shared.services.VersionComparison.isComposeVersionLessThan(
            ...
            if (shouldIncludeHotReloadFeature) {
                composer.addFeature(io.github.barsia.composewizard.composer.features.HotReloadFeature())
            }
