...
class LifecycleVersionService {
    ...
    companion object {
        private const val MAVEN_URL = "https://repo1.maven.org/maven2/org/jetbrains/androidx/lifecycle/lifecycle-runtime/"
        private const val TIMEOUT_MS = 5000
    }
    
    /**
     * Fetch all available Lifecycle versions from Maven Central.
     * 
    ...
    suspend fun fetchLifecycleVersions(): List<String> = withContext(Dispatchers.IO) {
        try {
            ...
            if (connection.responseCode == 200) {
                ...
                logger.warn("Failed to fetch Lifecycle versions: HTTP ${connection.responseCode}")
                emptyList()
            }
        } catch (e: Exception) {
            logger.warn("Failed to fetch Lifecycle versions: ${e.message}")
            emptyList()
        }
    }
