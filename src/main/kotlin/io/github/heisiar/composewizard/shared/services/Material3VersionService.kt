 ...
 */
class Material3VersionService {
    
    private val logger = Logger.getInstance(Material3VersionService::class.java)
    
    companion object {
        private const val MAVEN_URL = "https://repo1.maven.org/maven2/org/jetbrains/compose/material3/material3/"
        private const val TIMEOUT_MS = 5000
    }
    
    /**
     * Fetch all available Material3 versions from Maven Central.
     * 
    ...
    suspend fun fetchMaterial3Versions(): List<String> = withContext(Dispatchers.IO) {
        try {
            ...
            if (connection.responseCode == 200) {
                ...
                logger.warn("Failed to fetch Material3 versions: HTTP ${connection.responseCode}")
                emptyList()
            }
        } catch (e: Exception) {
            logger.warn("Failed to fetch Material3 versions: ${e.message}")
            emptyList()
        }
    }
