...
/**
 * Service for fetching Navigation (v2) library versions from Maven Central.
 * 
 * Note: Navigation v2 is available only for Compose versions < 1.10.0
 * For Compose 1.10.0+, use Navigation3 instead.
 * 
...
class NavigationVersionService {
    ...
    companion object {
        private const val MAVEN_URL = "https://repo1.maven.org/maven2/org/jetbrains/compose/material/material-navigation/"
        private const val TIMEOUT_MS = 5000
    }
    
    /**
     * Fetch all available Navigation versions from Maven Central.
     * 
    ...
    suspend fun fetchNavigationVersions(): List<String> = withContext(Dispatchers.IO) {
        try {
            ...
            if (connection.responseCode == 200) {
                ...
                logger.warn("Failed to fetch Navigation versions: HTTP ${connection.responseCode}")
                emptyList()
            }
        } catch (e: Exception) {
            logger.warn("Failed to fetch Navigation versions: ${e.message}")
            emptyList()
        }
    }
