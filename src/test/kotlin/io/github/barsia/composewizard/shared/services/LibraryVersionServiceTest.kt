...
class LibraryVersionServiceTest {
    ...
    fun `fetchVersions returns non-empty list for Material3`() = runBlocking {
        val metadata = LibraryRegistry.getMetadata(io.github.barsia.composewizard.shared.LibraryType.MATERIAL3)
    ...
    fun `fetchVersions returns non-empty list for Navigation`() = runBlocking {
        val metadata = LibraryRegistry.getMetadata(io.github.barsia.composewizard.shared.LibraryType.NAVIGATION)
        val versions = service.fetchVersions(metadata.mavenMetadataUrl)
    ...
    fun `fetchVersions returns non-empty list for Lifecycle`() = runBlocking {
        val metadata = LibraryRegistry.getMetadata(io.github.barsia.composewizard.shared.LibraryType.LIFECYCLE)
        val versions = service.fetchVersions(metadata.mavenMetadataUrl)
